...
# Superset stack backed by a local Postgres container + fake credentials. This
# mirrors setup.sh, but replaces the Neon branch with a docker-compose bundle
# (Postgres + neon-proxy + Electric) on allocated ports so worktrees do not
# collide.
set -uo pipefail
...
local_check_dependencies() {
  ...
  command -v caddy &> /dev/null || warn "caddy not found - Electric HTTPS proxy will not work (brew install caddy && caddy trust)"
