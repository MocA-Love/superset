 ...
 *
 * `agentId` is the wrapper-level id and matches `BuiltinAgentId`/`PRESET_ICONS`.
 * `definitionId` is the user-customized id when the launch path stamps it; it's
 * reserved for a future PR — wrappers can't distinguish user definitions on
 * their own.
 */
