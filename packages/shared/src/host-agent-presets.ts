 ...
 * renders. Lives here because it is static configuration that ships with
 * the binary, not data the API owns.
 *
 * Launch resolution:
 *   prompt
 ...
 *     : [command, ...args]
 *
 * `promptArgs` is only included when launching with a prompt, so codex's
 * trailing `--`, opencode's `--prompt`, and copilot's `-i` do not appear in
 * promptless launches. Stdin transport pipes the prompt to the spawned
 * process's stdin instead of pushing it to argv.
 *
 * Superset is intentionally excluded. Its model/provider config lives in chat
 * settings, not in terminal-agent configs.
 */
