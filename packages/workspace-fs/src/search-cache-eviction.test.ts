 ...
 *   - explicit invalidation still clears cached indexes
 *
 ...
 * after eviction is one ripgrep/fast-glob walk (~50-200 ms for a 5k-file repo).
 */
...
describe("searchIndexCache LRU eviction", () => {
	...
	it("evicts the least-recently-used entry when adding a (CACHE_MAX+1)th worktree", async () => {
		...
		// Entry 0 was evicted, then rebuilt — different object reference.
		expect(refetchedRoot0).not.toBe(initialReads[0]);
		expect(refetchedRoot0.items.length).toBe(2);
	});
