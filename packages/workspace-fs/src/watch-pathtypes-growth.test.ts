 ...
 *
 * `WatcherState.filePaths` accumulates one entry per file path that has been
 * seen via `create` / `update` / `rename` events. Delete events remove entries.
 * New file paths created over time (logs, build artifacts that escape
 * DEFAULT_IGNORE_PATTERNS, generated assets, dev-server tmp files) can grow
 * monotonically without a cap.
 *
 * Uses a real `FsWatcherManager` with the real `@parcel/watcher` backend and
 * real fs writes. Reaches into private `watchers` map after events flush to
 * assert file path growth — the same pattern other tests in this repo use to
 * inspect manager-internal state.
 */
...
async function createTempRoot(): Promise<string> {
	...
	// inside `filePaths` match what we compute in the test.
...
describe("FsWatcherManager.filePaths growth", () => {
	it("creating N new files adds N entries to filePaths", async () => {
	...
	it("updates to existing files do not grow filePaths", async () => {
		const rootPath = await createTempRoot();
		tempRoots.push(rootPath);
		...
		// Now mutate the same file 10 times. filePaths should not grow.
		for (let i = 0; i < 10; i++) {
			await fs.writeFile(filePath, `v${i + 2}`);
		}
		await waitForCondition(
			() => events.filter((kind) => kind === "update").length >= 1,
		);
	...
	it("delete events remove entries; create-new events add fresh ones", async () => {
		...
		await waitForCondition(
			() => events.filter((event) => event.startsWith("create:")).length >= 5,
		);

		const sizeAfterPhase1 = getPathTypes(manager, rootPath).size;
		expect(sizeAfterPhase1).toBeGreaterThanOrEqual(5);

		// Phase 2: delete those 5 files. filePaths should shrink.
		for (let i = 0; i < 5; i++) {
			await fs.rm(path.join(rootPath, `phase1-${i}.txt`));
		}
		await waitForCondition(
			() => events.filter((event) => event.startsWith("delete:")).length >= 5,
		);

		const sizeAfterDeletes = getPathTypes(manager, rootPath).size;
		expect(sizeAfterDeletes).toBeLessThan(sizeAfterPhase1);

		// Phase 3: create 5 NEW files with different names. filePaths grows
		// again. This is the leak shape: log rotation / dev-server tmp /
		// hashed build artifacts produce a stream of unique paths whose older
		// deletes happen sometime — but in the meantime filePaths climbs.
		for (let i = 0; i < 5; i++) {
			await fs.writeFile(path.join(rootPath, `phase3-${i}.txt`), "x");
		}
		await waitForCondition(
			() => events.filter((event) => event.startsWith("create:")).length >= 10,
		);
	...
	it("caps filePaths at filePathsMax — older entries evicted on overflow", async () => {
	...
	it("repeated create/delete with unique names grows filePaths monotonically until delete catches up", async () => {
		const rootPath = await createTempRoot();
		tempRoots.push(rootPath);
		...
		// Burst: create 30 unique paths before any delete fires. Without a cap,
		// filePaths would hold all 30 simultaneously.
