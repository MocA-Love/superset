...
/**
 * BENCHMARK companions to the integration tests for search index and watcher
 * path memory caps.
 *
 * Prints real heap deltas via `Bun.gc(true)` + `process.memoryUsage`, so
 * memory creep claims are anchored to measured JS heap numbers instead of
 * structural arguments alone. Output goes through `console.log`; assertions
 * are intentionally minimal so the benchmark does not fail on noisy machines.
 */
...
async function gcAndSample(): Promise<HeapSample> {
	...
	// can leave incremental work pending; double-pumping gets a more stable
	...
	test("prints heap MB after building 5/25/100 worktree indexes", async () => {
		...
		for (const stage of stages) {
			for (let i = 0; i < stage.count; i++) {
				...
				cumulativeEntries += index.items.length;
			}
...
describe("BENCH: filePaths heap delta vs unique paths", () => {
	test("prints heap MB after creating 1k/5k/20k unique paths in one worktree", async () => {
		...
		for (const target of stages) {
			...
			// Wait for parcel watcher to catch up. Once past the 10k file cap
			// the size plateaus, so cap the predicate target to avoid spinning
			// the deadline.
