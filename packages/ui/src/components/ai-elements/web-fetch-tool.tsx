...
export const WebFetchTool = ({
	...
	const statusNode = isPending ? (
		...
		<span className="text-muted-foreground text-xs">{formatBytes(bytes)}</span>
	) : null;
		...
		>
			{hasContent ? (
				<div className="max-h-[300px] overflow-y-auto">
					<pre className="whitespace-pre-wrap break-words px-2.5 py-2 font-mono text-foreground text-xs">
						{content}
					</pre>
				</div>
			) : undefined}
