...
export const nextEditConfigSchema = z.object({
	enabled: z.boolean(),
	model: z.string().min(1).default("mercury-coder"),
	maxTokens: z.number().int().min(1).max(16384),
	temperature: z.number().min(0.5).max(1.0),
...
function readPersistedNextEditConfig(
	...
	try {
		...
		console.warn(
			"[chat-service][next-edit] Failed to read persisted config.",
			{
				configPath,
