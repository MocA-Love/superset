...
function rollingDownloadBase(): string {
	return `https://github.com/${env.CLI_RELEASE_REPO}/releases/download/cli-latest`;
}

function releaseDownloadBase(version: string): string {
	return `https://github.com/${env.CLI_RELEASE_REPO}/releases/download/cli-v${version}`;
}
...
function detectTarget(): string {
	...
	throw new CLIError(
		...
		"The fork CLI release workflow currently publishes darwin-arm64 and linux-x64 builds.",
	);
}
...
async function fetchLatestVersion(): Promise<string> {
	const response = await fetch(`${rollingDownloadBase()}/version.txt`, {
		redirect: "follow",
	});
	if (!response.ok) {
		throw new CLIError(
			`Failed to fetch latest CLI version: ${response.status} ${response.statusText}`,
			`Expected a cli-latest release in ${env.CLI_RELEASE_REPO}.`,
		);
	}
...
function tarballUrl(target: string, version?: string): string {
	if (!version) {
		return `${rollingDownloadBase()}/superset-${target}.tar.gz`;
	}
	return `${releaseDownloadBase(version)}/superset-${target}.tar.gz`;
}
...
export default command({
	...
	run: async ({ options }) => {
		...
		if (options.check) {
			return {
				data: {
					...
					repository: env.CLI_RELEASE_REPO,
				},
				message: upToDate
		...
		// Stage as a sibling of the install root so renameSync() stays
		// on the same filesystem; tmpdir() can be a different mount.
