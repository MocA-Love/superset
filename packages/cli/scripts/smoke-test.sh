#!/usr/bin/env bash
#
# Smoke-tests a built CLI distribution. This is the single local/CI entrypoint
# for checking that the packaged CLI, host-service bundle, PTY daemon, and
# native addons are all shippable.
#
...
# node-pty must resolve from the bundled tree, and a real PTY spawn must work
# against the bundled Node + prebuild.
(
	cd /tmp
	NODE_PATH="$DIST/lib/node_modules" DIST="$DIST" "$DIST/lib/node" -e '
		...
		setTimeout(() => {
			console.error("[smoke] pty spawn timeout");
			process.exit(1);
		}, 5000);
	'
)
...
# token, secret and sqlite DB keep it isolated. Reaching health.check means
# host-service.js and its module graph loaded.
...
CLOUD_API_URL="${CLOUD_API_URL:-${SUPERSET_API_URL:-https://api.superset.sh}}"

env ORGANIZATION_ID="$HSORG" \
	AUTH_TOKEN="smoke-test-token" \
	CLOUD_API_URL="$CLOUD_API_URL" \
	SUPERSET_API_URL="$CLOUD_API_URL" \
