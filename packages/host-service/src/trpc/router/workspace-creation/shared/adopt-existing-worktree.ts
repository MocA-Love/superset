...
export interface AdoptExistingWorktreeResult {
	...
	/**
	 * True when an existing cloud row was reused; false when a new row was created
	 * in this call. Used by workspaces.create to decide whether to dispatch setup
	 * terminal and sugar agents.
	 */
 ...
 * all the stale-row hygiene so callers do not need to duplicate it.
 *
 * Cross-project safety is the caller's responsibility. Only pass a worktreePath
 * that came from git worktree list on this project's git client.
 */
