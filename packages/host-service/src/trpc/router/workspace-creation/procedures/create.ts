...
export const create = protectedProcedure
	...
	.mutation(async ({ ctx, input }) => {
		...
		// FORK NOTE: safeResolveWorktreePath uses (repoPath, branch) — fork variant.
		// Worktrees live under <repoPath>/.worktrees/<branch>, not the global
		// ~/.superset/worktrees/<projectId>/<branch> used by upstream split.
		...
		// Upstream split uses `startPoint.kind !== "head"` which would write
		// for local branches too — we intentionally diverge here.
		if (startPoint.kind === "remote-tracking") {
			await git
