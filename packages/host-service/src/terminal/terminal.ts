...
export function attachTerminalViewer(
	...
	return {
		...
		resize(cols, rows) {
			...
			notifyViewersResize(session, normalizedCols, normalizedRows);
		},
		runCommand(command) {
			if (detached || session.exited) return;
