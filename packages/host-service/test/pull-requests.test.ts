...
describe("PullRequestRuntimeManager branch sync", () => {
	test("persists unborn branches even when HEAD has no commit", async () => {
		...
		// The sweep routes through enqueueWorkspaceSync -> syncOneWorkspace,
		// which re-reads each workspace via `select().from().where().get()`.
		// Bypass the drizzle .where() chain by feeding the known row directly;
		// syncWorkspaceRow still runs the production logic.
		(
