...
describe("workspaceCreation.adopt integration", () => {
	...
	test("rejects with PROJECT_NOT_SETUP when project is not in db", async () => {
	...
	test("rejects with NOT_FOUND when explicit worktreePath is not registered", async () => {
		const scenario = await createProjectScenario({
			hostOptions: { apiOverrides: cloudFlows.workspaceCreateOk() },
		});
		dispose = scenario.dispose;
	...
	test("adopts a worktree at an explicit path, creates cloud row and local row", async () => {
		const scenario = await createProjectScenario({
			hostOptions: { apiOverrides: cloudFlows.workspaceCreateOk() },
		});
		dispose = scenario.dispose;
