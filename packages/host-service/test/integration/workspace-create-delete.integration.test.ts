...
describe("workspace.create and workspace.delete integration", () => {
	...
	test("create adopts an existing worktree at a non-canonical path", async () => {
		const scenario = await createProjectScenario({
			hostOptions: { apiOverrides: cloudFlows.workspaceCreateOk() },
		});
		dispose = scenario.dispose;
	...
	test("create adopts a worktree created by another tool", async () => {
		const scenario = await createProjectScenario({
			hostOptions: { apiOverrides: cloudFlows.workspaceCreateOk() },
		});
		dispose = scenario.dispose;
	...
	test("create with explicit worktreePath reads the current branch from git", async () => {
		const scenario = await createProjectScenario({
			hostOptions: { apiOverrides: cloudFlows.workspaceCreateOk() },
		});
		dispose = scenario.dispose;
	...
	test("create prunes a stale worktree before adding a new one", async () => {
		const scenario = await createProjectScenario({
			hostOptions: { apiOverrides: cloudFlows.workspaceCreateOk() },
		});
		dispose = scenario.dispose;
	...
	test("delete removes the worktree and local row on success", async () => {
