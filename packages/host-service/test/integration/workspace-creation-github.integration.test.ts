...
describe("workspaceCreation github procedures with mocked Octokit", () => {
	...
	test("searchGitHubIssues handles direct # lookup via issues.get", async () => {
	...
	test("searchGitHubIssues falls through to search for free text", async () => {
		const result = await host.trpc.workspaceCreation.searchGitHubIssues.query({
			projectId,
			query: "fix bug",
		});
	...
	test("searchPullRequests handles direct # lookup via pulls.get", async () => {
...
describe("resolveGithubRepo trusts the live local remote", () => {
	let host: TestHost;
	let repoDir: string;
	const projectId = randomUUID();
	...
	test("searchPullRequests routes against the local remote owner/name", async () => {
		const result = await host.trpc.workspaceCreation.searchPullRequests.query({
			projectId,
			query: "#33",
		});
...
describe("resolveGithubRepo prefers the user-configured remoteName", () => {
	...
	test("searchPullRequests routes against upstream, not origin", async () => {
		...
		expect(result.pullRequests).toHaveLength(1);
		expect(result.pullRequests[0]?.url).toContain(
			"github.com/upstream-org/cli",
		);
	});
});

describe("both GitHub backends failing rethrows", () => {
	let host: TestHost;
	let repoDir: string;
	const projectId = randomUUID();
	...
	test("searchPullRequests rethrows when gh and Octokit fail", async () => {
		await expect(
	...
	test("searchGitHubIssues rethrows when gh and Octokit fail", async () => {
		await expect(
...
describe("resolveGithubRepo requires a local clone", () => {
	let host: TestHost;
	const projectId = randomUUID();
...
describe("gh CLI is first-class when execGh succeeds", () => {
	...
	test("searchPullRequests free text invokes gh api with is:pr filter", async () => {
		const result = await host.trpc.workspaceCreation.searchPullRequests.query({
			projectId,
			query: "find me",
		});
	...
	test("searchGitHubIssues free text invokes gh api with is:issue filter", async () => {
		const result = await host.trpc.workspaceCreation.searchGitHubIssues.query({
			projectId,
			query: "bug",
		});
