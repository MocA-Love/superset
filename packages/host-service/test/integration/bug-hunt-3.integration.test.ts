...
describe("bug-hunt-3: branch-name path traversal in workspaces.create", () => {
	let host: TestHost;
	let repo: GitFixture;
	...
	test("workspaces.create with a '../escape' branch name is rejected (no worktree outside project)", async () => {
		...
		const expectedEscapePath = join(repo.repoPath, ".worktrees", evilBranch);

		const result = await host.trpc.workspaces.create
			.mutate({
				projectId,
				name: "x",
				branch: evilBranch,
...
describe("bug-hunt-3: branch-name path traversal in workspaceCreation.create", () => {
	test.todo(
		"workspaceCreation.create was removed from this fork; keep validation coverage on workspaces.create",
	);
});
...
describe("bug-hunt-3: more concurrency probes", () => {
	...
	test("parallel workspaces.create calls for different branches do not race on the same .git/config", async () => {
		host = await createTestHost({
			apiOverrides: {
				"host.ensure.mutate": () => ({ machineId: "m1" }),
				"v2Workspace.create.mutate": (input: unknown) => {
		...
		if (failures.length > 0) {
			console.warn(
				"[bug-hunt-3] parallel workspaces.create failure(s):",
