...
describe("workspaceCleanup.destroy integration", () => {
	...
	test("rejects deleting a main workspace by path equality", async () => {
		await expect(
	...
	test("blocks on dirty worktree with CONFLICT when force is false", async () => {
	...
	test("force=true skips preflight and runs cloud delete plus db cleanup", async () => {
		writeFileSync(join(scenario.worktreePath, "dirty.txt"), "uncommitted");
	...
	test("missing locked worktree is removed without warnings", async () => {
		await scenario.repo.git.raw(["worktree", "lock", scenario.worktreePath]);
		rmSync(scenario.worktreePath, { recursive: true, force: true });
