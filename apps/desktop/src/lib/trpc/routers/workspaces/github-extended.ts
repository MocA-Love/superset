...
async function getGitHubRepositoryOverviewForWorkspace(workspaceId: string) {
	const {
		repoPath,
		repositoryNameWithOwner,
		repositoryUrl,
...
export const createGithubExtendedRouter = () =>
	router({
		getPullRequestIdentityCandidates: publicProcedure
			...
			.query(async ({ input }) => {
				return getPullRequestIdentityCandidatesHelper(input);
			}),
		...
		getGitHubRepositoryOverview: publicProcedure
			...
			.query(async ({ input }) => {
				return getGitHubRepositoryOverviewForWorkspace(input.workspaceId);
			}),
