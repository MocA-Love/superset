...
export function usePierreRowClickPolicy({
	...
	const onClickCapture = useCallback(
		(e: React.MouseEvent<HTMLDivElement>) => {
			...
			// Always intercept. Pierre's selectOnlyPath no-ops when the clicked
			// row is already selected, which silently drops legitimate re-clicks
			// such as click-to-pin or reopening a file after Cmd+W.
			e.preventDefault();
			e.stopPropagation();
