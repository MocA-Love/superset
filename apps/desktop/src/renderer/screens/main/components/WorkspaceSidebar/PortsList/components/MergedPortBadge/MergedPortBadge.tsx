...
export function MergedPortBadge({ port }: MergedPortBadgeProps) {
	...
	// FORK NOTE: v1 PortsList lists both detected and configured-but-undetected
	// static ports. `isDetected` toggles colour scheme and gates kill/open
	// affordances. `canJumpToTerminal` mirrors the legacy paneId hint, now
	// renamed to terminalId after the upstream port-scanner extraction.
	const isDetected = port.detected;
	const canJumpToTerminal = !!port.terminalId;
	...
	return (
		<Tooltip>
			<TooltipTrigger asChild>
				<div
					className={cn(
						"group relative mb-1 inline-flex max-w-full items-center gap-1 rounded-md text-xs transition-colors",
						isDetected
