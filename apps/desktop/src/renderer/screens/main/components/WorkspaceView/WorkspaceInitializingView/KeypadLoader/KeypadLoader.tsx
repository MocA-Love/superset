...
export function KeypadLoader({
	...
	// Reduced-motion implies reduced-audio. Always auto-mute when it is on,
	// even if the caller did not pass muted.
	...
	useEffect(() => {
		...
		// Cap rapid-fire clicks on a huge step skip to avoid audio spam.
		...
		for (let i = 0; i < clicksToPlay; i++) {
			const id = window.setTimeout(() => {
				try {
					...
					// notification-mute setting after this click was scheduled.
					const current = audioRef.current;
					if (!current || current.muted) return;
					// Clone per click so overlapping plays do not cancel each other
