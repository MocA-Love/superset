...
// setupKeyboardHandler (upstream #3724 migration).
...
export interface KeyboardHandlerOptions {
	...
	// FORK NOTE: shell history suggestion integration (PR #<suggestion-feature>).
	// Not present in upstream — these drive the TerminalSuggestion overlay.
	// Follow-up: apply to v2 terminal if needed.
 ...
 * - Line-edit chord translations (shared with v2 via translateLineEditChord)
 * - Shell history suggestion navigation (fork-only)
 *
...
export function setupKeyboardHandler(
	...
	const handler = (event: KeyboardEvent): boolean => {
		...
		// FORK NOTE: suggestion overlay key handling — intercepts arrow keys
		// and Enter when a suggestion is active before the shell sees them.
