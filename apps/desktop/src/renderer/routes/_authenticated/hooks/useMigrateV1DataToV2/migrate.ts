...
export async function migrateV1DataToV2(args: Args): Promise<MigrationSummary> {
	...
	for (const workspace of v1Workspaces) {
		...
		const v1Worktree = workspace.worktreeId
			...
			: undefined;
		// FORK NOTE: branch-type workspaces (no worktreeId) fall back to the
		// parent project's mainRepoPath so adopt() can resolve the branch
		// inside the primary checkout. listWorktreeBranches excludes the
		// primary working tree, otherwise these workspaces skip with
		// "worktree_not_registered".
		const v1WorktreePath =
			v1Worktree?.path ??
