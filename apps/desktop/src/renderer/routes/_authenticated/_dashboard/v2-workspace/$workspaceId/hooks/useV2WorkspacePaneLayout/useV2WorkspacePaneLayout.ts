...
export function useV2WorkspacePaneLayout() {
	...
	// one store across that boundary lets panes from one worktree render and
	// persist under another.
	...
	// FORK NOTE: page.tsx reads localWorkspaceState.rightSidebarOpen for the
	// right-sidebar portal-slot wiring (cycle 09). Restore the export that
	// upstream #4067 dropped; everything else stays on the upstream signature.
	return { store, localWorkspaceState };
}
