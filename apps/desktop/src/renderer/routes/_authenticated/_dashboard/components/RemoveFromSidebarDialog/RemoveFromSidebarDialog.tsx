...
export function RemoveFromSidebarDialog() {
	const target = useRemoveFromSidebarIntent((state) => state.target);
	const clear = useRemoveFromSidebarIntent((state) => state.clear);
		...
		<AlertDialog open={!!target} onOpenChange={handleOpenChange}>
			<AlertDialogContent className="max-w-[360px] gap-0 p-0">
				<AlertDialogHeader className="px-4 pt-4 pb-2">
					...
					<AlertDialogDescription>
						The workspace itself is not deleted. You can add it back from the
						Workspaces page.
					</AlertDialogDescription>
				</AlertDialogHeader>
