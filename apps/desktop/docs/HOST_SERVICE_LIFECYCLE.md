...
Each host-service still writes `~/.superset/host/{orgId}/manifest.json` (pid, endpoint, authToken). Electron's coordinator no longer reads it for adoption; the manifest is now consumed by:
